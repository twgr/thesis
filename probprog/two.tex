...
Church is a PPL based on Scheme \cite{goodman_uai_2008}.  
The original seminal paper and accompanying system 
	...
	which returns true if the desired conditions are satisfied.  This is somewhat restrictive for practical
	use as one cannot effectively condition on continuous data, given that the generation of that data
	under the joint distribution has zero probability such that this post-hoc hard constraint application
	will never be satisfied.  This complete separation of the generative process and the conditioning (which
	is arguably more in line with approximate Bayesian computation~\citep{csillery2010approximate}
	 methods than with modern PPLs)
	can also be wasteful in not allowing the structure of a model to be exploited (see Chapters~\ref{chp:part} and
	\ref{chp:proginf}).  Later systems therefore mostly allow soft conditioning statements to be interleaved
	through the generative progress (in an analogous manner to likelihood terms), increasing the range
	of models than can be encoded and potentially efficiency of inference algorithms.
