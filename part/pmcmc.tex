...
algorithm is now demonstrated by showing that it is a valid MH sampler for an \emph{extended target distribution}
...
\begin{enumerate}
	...
	except for having one particle and path, $\{\xb^{b},\bb\}$, already predefined.
\end{enumerate}
We can informally think of this as doing the PIMH process in reverse.  Rather than
running an SMC sweep and sample one of the produced particles to produce $\{\xb^{b},\bb\}$, we
first sample the $\{\xb^{b},\bb\}$ and then the rest of the variables in the SMC sweep conditioned
...
and choosing one particle $b$ implies a particular $\{\xb^{b},\bb\}$.
