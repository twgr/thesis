...
MH sampler on an \emph{extended target distribution} whose marginal distribution on $\xb^b$ is the distribution of interest $\pi(\xb^b)$.
...
\begin{enumerate}
	...
	\item Sample all of the other particles and ancestor indices conditioned on $\{\xb^{b},\bb\}$ using \begin{align}
	\label{eq:part:csmc}
	...
	except for having one particle and path, $\{\xb^{b},\bb\}$, predefined.
\end{enumerate}
We can informally think of this as doing the PIMH process in reverse.  Rather than
running an SMC sweep and sampling one of the produced particles to produce $\{\xb^{b},\bb\}$, it
first samples the $\{\xb^{b},\bb\}$ and then the rest of the variables in the SMC sweep conditioned
...
\begin{align}
\int \tilde{\pi}(\xi,b) d\xi = \int q_{\text{PIMH}}(\xib,b) \frac{\hat{Z}}{Z} d\xi
= p(b|\xi)\int q_{\text{SMC}}(\xib) \frac{\hat{Z}}{Z} d\xi
= \bar{w}_T^b \int q_{\text{SMC}}(\xib) \frac{\hat{Z}}{Z} d\xi = \bar{w}_T^b
\end{align}
...
our target as it can change the retained particle, the index $b$, and all the other particles.\footnote{Note
	...
	system and we need not worry about the possibility of the retained particle simply being ``passed around''.}
...
Degeneracy can be especially detrimental when observations late in the sequences have noticeable
...
never detrimental.  At a high level, we are taking an analytic expectation over part of
