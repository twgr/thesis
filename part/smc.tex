...
We note that some PMCMC methods, namely particle independent Metropolis-Hastings and particle marginal
Metropolis-Hastings, can be interpreted as pseudo-marginal methods.
%
...
We finish the section by considering what the optimal proposals are for SMC.  Here we can
leverage ideas from the effective sample size and consider what proposal minimizes the variance of the estimate
for the next target.  It trivial to see that this is
\begin{align}
q^*(x_t | x_{1:t-1}) = p(x_t | x_{1:t-1}, y_t)\propto f_t(x_t | x_{1:t-1}) g_t(y_t | x_{1:t})
\end{align}
for the NMSSM formulation as this results in all of the weights being equal, giving an effective sample size of $N$.
\todo{Wrong as different normalization constants.}
This as known as the \emph{one-step optimal proposal} as it is the optimal proposal, in terms of
variance of the estimator, for estimating the next target~\citep{doucet2009tutorial} (i.e. the filtering distribution).
Though theoretically establishing the optimal proposal for the smoothing distribution is somewhat more
complicated, the one-step optimal proposal is still generally best in practice.  In particular, it is generally
...
There is not generally an analogue to the one-step optimal proposal for SMC with an arbitrary series of targets
because whereas $\int p(x_t | x_{1:t-1}, y_t) dx_t=1$ regardless of the current sample for $x_{1:t-1}$,
\todo{Wrong because can do the same thing with arbitrary targets}
\todo{Looks like it is optimality in terms of variance of each individual weight, not the variance in terms of the ESS definition.  However, if the variance of each weight is 0, the variance of the
	weights as a set is zero, and thus can do resampling first (if we knew %$p(y_t | x_{1:t-1})$, and 
	thus we destroy less information by doing the resampling earlier.}