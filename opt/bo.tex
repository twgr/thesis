...
original function, for example optimizing the log probability in MAP problem~\citep{osborne2010bayesian}.
...
problem that cannot be solved simply by, for example, Monte Carlo~\citep{rainforth2017pitfalls}.
...
$p\left(V | \Theta, \alpha\right)$ is the GP marginal likelihood as given in~\eqref{eq:opt:GP-ML}.
...
for scenarios where it is essential that the constraints are never violated, for example if the constraints
correspond to the failure of a component.
In recent work that will be discussed at length in Chapter~\ref{chp:bopp}~\citep{rainforth2016bayesian}, we show how
