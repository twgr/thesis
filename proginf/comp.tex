...
that its the CPS transformation is \clj{(defn f& [args} ~$\mP$\clj{] (}$\mP$ \clj{(f args)))}.\footnote{Note that
	...
	for functions with variable number of inputs.}
  We 
...
at run time.  Secondly, its creates an abstraction barrier for writing inference algorithms -- implementing
...
\end{lstlisting}\vspace{-8pt}
where \clj{algorithm} specifies the inference algorithm; \clj{prog} is program to call, comprising of either the full
...
represented by anonymous functions with no inputs excepts for our checkpoints, this will run until it
...
\clj{algorihtm} and the type of \clj{step} (e.g. calling to the \sample checkpoint method of an algorithm 
...
We have now demonstrated how an Anglican query is compiled, an summarizing example for which
...
we might have to re-implement for a particular inference algorithm, namely ones to dictate the behavior
