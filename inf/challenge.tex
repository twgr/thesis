...
for the posterior as it allows use of Monte Carlo
 ...
 can in turn be used to carry out many of the outlined tasks.  For example, they can be
 used to estimate an expectation via Monte Carlo estimation
\begin{align}
	\label{eq:inf:mc-est}
\E_{p(\theta|\mathcal{D})}\left[f(\theta)\right] \approx \frac{1}{N} \sum_{n=1}^{N}f(\hat{\theta}_n)
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.7\textwidth]{inf_example}
