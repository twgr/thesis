...
most adversarial possible function for that proposal, under the assumption that the function
values are independent for different points, i.e. that knowing $f(\theta)$ provides no information
about $f(\theta+\varepsilon)$.  Assume further that we are in the pre-normalized case so that
our estimator is unbiased.
Now imagine that we have $N$ independently drawn weighted samples
$\{\hth_n,w_n\}_{n=1:N}$, which thus have $N$ corresponding evaluations $f_n := f(\hth_n)$.
By the unbiasedness of the estimator, we have that 
\begin{align}
\norm{I_N-I}_2^2 &= \E \left[\left(\sum_{n=1}^{N} w_n f_n\right)^2\right].
\end{align}
We further presume that $\sum_{n=1}^{N}w_n = 1$ and $\sum_{n=1}^{N} \|f_n\| = 1$ to
preclude the ability to provide an improved solution simply by scaling the problem.
Under these restrictions, we see that the worst case set of $f_n$ is when $f_{n^*}=1$
and $f_n = 0, \; \forall n\neq n^*$ where $n^*$ is the index of the largest weight.
The set of weights that minimizes the variance under the worst case $\{f_n\}_{n=1}^{N}$
is then for all the weighs to be equal, as this minimizes $w_{n^*} f_{n^*}$.  Consequently,
